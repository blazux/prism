...
Les agents de groupe utilisent la même configuration de déploiement. Embeddings configurables via la même connexion ou une connexion indépendante, test réel de dimension, état de redémarrage nécessaire. Changement de modèle/endpoint : accord explicite et réindexation transactionnelle des textes stockés au prochain démarrage. La table `rag_embedding_meta` mémorise l’identité de l’index, y compris à dimension égale. Le repli visuel pour les widgets suit le modèle de conversation déclaré compatible vision ; les déploiements sans override conservent `VISION_MODEL`.
